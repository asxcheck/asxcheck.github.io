...
const CONFIG = {
    ...
    //   https://asx.mrlaurules.workers.dev
...
const CATALOG = [
    ...
    { symbol: 'CSL.AX', name: 'CSL Ltd', color: '#00d084' },
    { symbol: 'CBA.AX', name: 'Commonwealth Bank of Australia', color: '#f72585' },
    { symbol: 'WBC.AX', name: 'Westpac Banking Corp', color: '#ffd166' },
    { symbol: 'NAB.AX', name: 'National Australia Bank', color: '#f57c00' },
    { symbol: '360.AX', name: 'Life 360 Inc', color: '#84cc16' },
    { symbol: 'SHL.AX', name: 'Sonic Healthcare Ltd', color: '#118ab2' },
];
const state = {
...
function renderCatalogList(filter = '') {
    stockListEl.innerHTML = '<div class="listHeader" role="row">\n          <div>Ticker</div>\n          <div>Company</div>\n          <div>Last</div>\n          <div>Change</div>\n        </div>';
    const q = filter.trim().toLowerCase();
    CATALOG.forEach(item => {
        const match = !q || item.name.toLowerCase().includes(q) || item.symbol.toLowerCase().includes(q);
        if (!match) return;
        const symbol = item.symbol;
        const isSelected = state.primarySymbol === symbol;
        const row = document.createElement('div');
        row.className = 'stockItem' + (isSelected ? ' primary' : '');
        const price = state.prices[symbol];
        const open = state.openingPrice[symbol] ?? price;
        const delta = price != null && open != null ? price - open : 0;
        const deltaPct = open ? (delta / open * 100) : 0;
        row.innerHTML = `
          <div class="colSymbol">${symbol}</div>
          <div class="colName" title="${item.name}">${item.name}</div>
          <div class="colLast" aria-label="Current price for ${symbol}"><span id="latest-${symbol}" class="price">${price != null ? fmt(price) : '-'}</span></div>
          <div class="colChange"><span class="deltaBadge" style="background:${delta >= 0 ? 'rgba(39,199,130,.15)' : 'rgba(255,92,92,.15)'}; color:${delta >= 0 ? 'var(--green)' : 'var(--red)'}">${delta >= 0 ? '▲' : '▼'} ${isFinite(deltaPct) ? fmt(deltaPct) : '0.00'}%</span></div>
        `;
        stockListEl.appendChild(row);
        // Row click selects this symbol only
        row.addEventListener('click', () => setPrimary(symbol));
    });
}
...
async function loadRangeForPrimary(rangeKey) {
    ...
    const sym = state.primarySymbol;
    try {
        const points = await fetchYahooChartHistory(sym, range, interval);
        if (!points || points.length === 0) return;
        state.history[sym] = points.slice();
        state.prices[sym] = points[points.length - 1].price;
        state.volumes[sym] = points[points.length - 1].volume;
